...
implied_volatility = bsm.implied_vol(
    observed_call_price=observed_call_price, num_iterations=1000, tolerance=1e-4
)

# Calculate put price
put_price_cal = bsm.put_value()  # using calculated call price
put_price_obs = bsm.put_value(
    observed_call_price=observed_call_price
)  # using observed call price
...
jump_size_std = 0.5
option_type = "call"
call_price_merton_jump_diffusion = bsm.merton_jump_diffusion(
...
lookback_call = bsm.lookback_BSM(
    option_type="call", max_share_price=150, min_share_price=50
)
...
cash_or_nothing_barrier = bsm.cash_or_nothing_barrier_options(
    option_type="call",
    barrier_price=35,
...
print(
    "Call price with Merton jump diffusion model: %.3f"
    % call_price_merton_jump_diffusion
)
