...
# %%
# Drive both pivot tables from one slicer
...
# %%
# ChangePivotCache raises com_error if a slicer cache is connected to the pivot.
# Must delete the slicer cache first, swap caches, then recreate.
wb.api.SlicerCaches(1).Delete()
...
# Recreate slicer after cache swap
sc = wb.api.SlicerCaches.Add2(
    Source=ws_pivot.api.PivotTables("TitanicPivot"),
    SourceField="pclass",
)
sc.PivotTables.AddPivotTable(ws_pivot.api.PivotTables("TitanicPivot2"))
slicer = sc.Slicers.Add(SlicerDestination=ws_pivot.api, Width=100, Height=200)
slicer.Top = 90
slicer.Left = 900
