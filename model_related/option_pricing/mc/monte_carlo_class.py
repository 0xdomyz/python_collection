...
class MonteCarloOptionPricing:
    def __init__(
        ...
        no_of_slices: int = 4,
        fix_random_seed: bool or int = False,
    ):
        """
        ...
        self.mue = (
            r  # under risk-neutral measure, asset expected return = risk-free rate
        )
        ...
        self.z_t = np.random.standard_normal(
            (self.simulation_rounds, self.no_of_slices)
        )
    ...
    def vasicek_model(self, a: float, b: float, sigma_r: float) -> np.ndarray:
        ...
        :return:
        """
        _interest_z_t = np.random.standard_normal(
            (self.simulation_rounds, self.no_of_slices)
        )
        _interest_array = np.full(
            (self.simulation_rounds, self.no_of_slices), self.r[0, 0] * self._dt
        )

        for i in range(1, self.no_of_slices):
            _interest_array[:, i] = (
                b
                + np.exp(-a / self.no_of_slices) * (_interest_array[:, i - 1] - b)
                + np.sqrt(
    ...
    def cox_ingersoll_ross_model(
        self, a: float, b: float, sigma_r: float
    ) -> np.ndarray:
        """
        ...
        for i in range(1, self.no_of_slices):
            _Lambda = (
                4
                * a
                * np.exp(-a / self.no_of_slices)
                * _interest_array[:, i - 1]
                / (sigma_r**2 * (1 - np.exp(-a / self.no_of_slices)))
            )
            _chi_square_factor = np.random.noncentral_chisquare(
                df=_dof, nonc=_Lambda, size=self.simulation_rounds
            )

            _interest_array[:, i] = (
                sigma_r**2
                * (1 - np.exp(-a / self.no_of_slices))
                / (4 * a)
                * _chi_square_factor
            )
    ...
    def heston(
        self, kappa: float, theta: float, sigma_v: float, rho: float = 0.0
    ) -> np.ndarray:
        """
        ...
        _zt = sts.multivariate_normal.rvs(
            mean=_mu, cov=_cov, size=(self.simulation_rounds, self.no_of_slices)
        )
        _variance_array = np.full(
            (self.simulation_rounds, self.no_of_slices), self.sigma[0, 0] ** 2
        )
        self.z_t = _zt[:, :, 0]
        _zt_v = _zt[:, :, 1]
        ...
        for i in range(1, self.no_of_slices):
            ...
            _diffusion = (
                sigma_v
                * np.sqrt(_previous_slice_variance)
                * _zt_v[:, i - 1]
                * np.sqrt(self._dt)
            )
    ...
    def stock_price_simulation(self) -> np.ndarray:
        ...
        for i in range(1, self.no_of_slices):
            self.price_array[:, i] = self.price_array[:, i - 1] * np.exp(
                self.exp_mean[:, i - 1]
                + self.exp_diffusion[:, i - 1] * self.z_t[:, i - 1]
            )
        ...
        print(
            ...
            % (
                ...
                self.S0,
                self.K,
                np.max(self.terminal_prices),
                np.min(self.terminal_prices),
                self.stock_price_expectation,
            )
        )
        print("-" * 64)
    ...
    def stock_price_simulation_with_poisson_jump(
        self, jump_alpha: float, jump_std: float, poisson_lambda: float
        ...
        """
        self.z_t_stock = np.random.standard_normal(
            (self.simulation_rounds, self.no_of_slices)
        )
        self.price_array = np.zeros((self.simulation_rounds, self.no_of_slices))
        self.price_array[:, 0] = self.S0
        ...
        for i in range(1, self.no_of_slices):
            ...
            self.poisson_jump_factor = np.exp(
                self.m * (jump_alpha - 0.5 * jump_std**2)
                + jump_std * np.array(self.sum_w)
            )
            ...
            self.price_array[:, i] = (
                ...
                * np.exp(
                    self.exp_mean[:, i]
                    + self.exp_diffusion[:, i] * self.z_t_stock[:, i]
                )
    ...
    def european_call(self) -> Tuple[float, float]:
        ...
        self.expectation = np.average(
            self.terminal_profit * np.exp(-np.sum(self.r, axis=1))
        )
    ...
    def european_put(self, empirical_call: float or None = None) -> float:
        ...
        self.put_value = (
            self.european_call_value
            + np.exp(-np.sum(self.r, axis=1)) * self.K
            - np.exp(-self.div_yield * self.T) * self.S0
        )

        return self.put_value

    def asian_avg_price_option(
        self, avg_method: str = "arithmetic", option_type: str = "call"
    ) -> float:
        assert (
            option_type == "call" or option_type == "put"
        ), "option_type must be either call or put"
        assert len(self.terminal_prices) != 0, "Please simulate the stock price first"
        assert (
            avg_method == "arithmetic" or avg_method == "geometric"
        ...
        if avg_method == "arithmetic":
            ...
            self.expectation = sts.gmean(
                self.terminal_profit * np.exp(-np.sum(self.r, axis=1))
            )

        self.standard_error = np.std(self.terminal_profit) / np.sqrt(
            len(self.terminal_profit)
        )

        print("-" * 64)
        print(
            " Asian %s monte carlo arithmetic average \n S0 %4.1f \n K %2.1f \n"
            " Option Value %4.3f" % (option_type, self.S0, self.K, self.expectation)
        )
        print("-" * 64)

        return self.expectation

    def american_option_longstaff_schwartz(
        self, poly_degree: int = 2, option_type: str = "call"
    ) -> float:
        """
        ...
        for t in range(
            self.no_of_slices - 2, 0, -1
        ):  # fill out the value table from backwards
            ...
            itm_path = np.where(
                self.intrinsic_val[:, t] > 0
            )  # <==> self.price_array[:, t] vs. self.K
        ...
        simulation_vals = (
            self.intrinsic_val * stopping_rule * self.discount_table
        ).sum(axis=1)
    ...
    def barrier_option(
        ...
        parisian_barrier_days: int or None = None,
    ) -> float:
        assert (
            option_type == "call" or option_type == "put"
        ), "option type must be either call or put"
        assert (
            barrier_type == "knock-in" or barrier_type == "knock-out"
        ), "barrier type must be either knock-in or knock-out"
        assert (
            barrier_direction == "up" or barrier_direction == "down"
        ...
        if barrier_direction == "up":
            ...
            if parisian_barrier_days is not None:
                days_to_slices = int(
                    parisian_barrier_days * self.no_of_slices / (self.T * 252)
                )
                parisian_barrier_check = np.zeros(
                    (self.simulation_rounds, self.no_of_slices - days_to_slices)
                )
                for i in range(0, self.no_of_slices - days_to_slices):
                    parisian_barrier_check[:, i] = np.where(
                        np.sum(barrier_check[:, i : i + days_to_slices], axis=1)
        ...
        if barrier_type == "knock-in":
            self.terminal_profit = np.where(
                np.sum(barrier_check, axis=1) >= 1, self.intrinsic_val[:, -1], 0
            )
        elif barrier_type == "knock-out":
            self.terminal_profit = np.where(
                np.sum(barrier_check, axis=1) >= 1, 0, self.intrinsic_val[:, -1]
            )
